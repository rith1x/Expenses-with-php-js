window.onload = function() {
  ...
  var icon = document.getElementById("icox");
  link.addEventListener("click", function(event) {
    event.preventDefault();
    if (info.style.display === "none") {
      info.style.display = "block";
      icon.style.transform = "rotate(135deg)";
      icon.style.color = "#eb4334";
    }
    else {
...
function addTransactionDOM(transaction) {
  ...
  item.innerHTML = `
    ${transaction.date} | ${transaction.text} <span>${sign} Rs ${Math.abs(
    transaction.amount
  )}</span>
      <button id="myButton" class="delete-btn" onclick="removeTransaction(${transaction.id})"><i class="fa-solid fa-trash"></i></button>
      `;
